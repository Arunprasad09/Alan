...
public static void main(String[] args) {
	...
	WebElement ae = driver.findElement(By.xpath("(//p[@data-ux='Text'])[1]"));
	ae.click();
	driver.manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);
	WebElement prcss = driver.findElement(By.id("personalization-select-6"));
	Select s=new Select(prcss);
	s.selectByIndex(1);
	WebElement memory = driver.findElement(By.id("personalization-select-4"));
	Select s1=new Select(memory);
	s1.selectByIndex(2);
    WebElement str = driver.findElement(By.id("personalization-select-5"));
    Select s2=new Select(str);
    s2.selectByIndex(1);
    WebElement install = driver.findElement(By.id("personalization-select-1"));
    Select s3=new Select(install);
    s3.selectByValue("18");
